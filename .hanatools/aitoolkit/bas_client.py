...
@proxy_clients.register('gen-ai-bas-proxy')
class GenAIBASProxyClient(BaseProxyClient, extra='allow'):
    ...
    def init_client(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if data.get('ai_core_client', None) is not None:
                return data
            kwargs = {}
            if 'H2O_URL' in os.environ:
                kwargs['base_url'] = os.environ['H2O_URL'] + '/llm'
                for name in ['auth_url', 'client_id', 'client_secret']:
                    kwargs[name] = None
                kwargs['resource_group'] = 'default'
            else:
            ...
            if 'H2O_URL' in os.environ:
                data['ai_core_client'].rest_client.headers.update({'AI-Resource-Group': 'default'})
        return data
    ...
    def update_deployments(self):
        ...
        if 'H2O_URL' in os.environ:
            deployment_list = [*deployment_set.values()]
            for deployment in deployment_list:
                deployment.url = os.environ['H2O_URL'] + '/llm/v2/inference/deployments/' + deployment.deployment_id
            self._deployments = deployment_list
        else:
